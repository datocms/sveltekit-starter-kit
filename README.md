...
This bidirectional communication is established automatically when your preview runs inside the Web Previews plugin—no additional configuration needed **on the code side**. You do, however, need the Web Previews plugin itself installed and pointed at this app; see [Configuring Visual Editing manually](#configuring-visual-editing-manually) if that setup didn't happen automatically for you.
...
When you deploy this starter through the [DatoCMS marketplace](https://dashboard.datocms.com/deploy?repo=datocms%2Fsveltekit-starter-kit%3Amain), the Web Previews plugin (which powers Visual Editing) is installed and configured for you automatically by the [`src/routes/api/post-deploy/+server.ts`](src/routes/api/post-deploy/+server.ts) code.
...
Both endpoints are guarded by `PRIVATE_SECRET_API_TOKEN`; if the token in the URL doesn't match your environment variable, the requests are rejected.
...
> The same [`post-deploy`](src/routes/api/post-deploy/+server.ts) code also configures other integrations (such as the SEO/Readability Analysis plugin). Treat it as the authoritative reference if you need to set those up by hand too.
